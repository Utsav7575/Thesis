...
CSV_FILE = "ball_tracker_with_kalman_filter_advanced.csv"
OUT_PREFIX = "arcD_kalman_advanced"
...
plt.title("Architecture D: Position vs Time (All rows)")
plt.legend()
plt.tight_layout()
...
with open(f"{OUT_PREFIX}_pos_full.png.meta.json",'w') as f:
    json.dump({
        "caption":"Architecture D: x_norm & y_norm vs time (all rows)",
        "description":"Includes detected=0 rows from advanced Kalman filter."
    }, f, indent=2)
...
plt.xlabel("Time (s)")
plt.ylabel("Normalized position")
plt.title("Architecture D: Position vs Time (Detected Only)")
plt.legend()
plt.tight_layout()
...
with open(f"{OUT_PREFIX}_pos_detected.png.meta.json",'w') as f:
    json.dump({
        "caption":"Architecture D: x_norm & y_norm vs time (detected only)",
