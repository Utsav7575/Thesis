...
smoothing_window = 5     # Number of previous positions for smoothing

# Serial communication settings
ARDUINO_PORT = '/dev/cu.usbmodem14201'  # Your MacBook Arduino port
BAUD_RATE = 9600
...
def send_to_controller(center_x, center_y, ball_detected, frame_count):
    """Send coordinates to controller - modify this for your specific controller"""
    # Only print every 30 frames (roughly once per second at 30 FPS)
    if frame_count % 30 == 0:
        if ball_detected:
...
while True:
    ...
    # Frame processing based on settings
    ...
    # Create mask for the ball color
    ...
    if contours:
        ...
        if contour_area > min_contour_area:
            ...
            if M["m00"] > 0 and radius > min_radius:
                ...
                # Send coordinates to controller (using centered coordinates)
                send_to_controller(x_centered, y_centered, True, frame_count)

    # Handle ball lost scenario
    if not ball_detected:
        ball_lost_counter += 1
        
        # Clear previous centers if ball lost for too long
        ...
        # Send "no ball" signal to controller
        send_to_controller(0.0, 0.0, False, frame_count)

    # Display frame info
    cv2.putText(frame, f"Frame: {frame_width}x{frame_height}", (frame_width - 200, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
    cv2.putText(frame, f"FPS Target: {desired_fps}", (frame_width - 200, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
